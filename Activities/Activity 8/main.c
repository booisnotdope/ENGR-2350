...
uint8_t time[4];
uint8_t flag = 0;
const uint8_t pins = GPIO_PIN0 | GPIO_PIN2 | GPIO_PIN3 | GPIO_PIN5 | GPIO_PIN6 | GPIO_PIN7;
...
void Timer_Init(){
    ...
    timer.clockSourceDivider = TIMER_A_CLOCKSOURCE_DIVIDER_32;
    timer.timerPeriod = 0.01 / (1.0 / (3.0/4.0 * pow(10,6)));
...
//     timer.timerPeriod = 0.01 / (1.0 / (1.0/2.0 * pow(10,6)));
...
void GPIO_Init(){
    ...
    //bumper buttons
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P4, pins);

    //registering interrupt
    GPIO_registerInterrupt( GPIO_PORT_P4 , Port4_ISR );
    GPIO_interruptEdgeSelect( GPIO_PORT_P4 , pins , GPIO_HIGH_TO_LOW_TRANSITION );
    GPIO_enableInterrupt( GPIO_PORT_P4 , pins );
}
...
void Port4_ISR(){
    ...
    uint16_t active_pins = GPIO_getEnabledInterruptStatus( GPIO_PORT_P4 );
    while(GPIO_getEnabledInterruptStatus( GPIO_PORT_P4 )){}
