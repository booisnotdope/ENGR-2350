...
void Heading_Control(){
    ...
    error_heading = DESIRED_HEADING - measured_heading;
    if (error_heading > 1800){
        error_heading -= 3600;
    } else if (error_heading < -1800){
        error_heading += 3600;
    }
    differential = kp_heading * error_heading;
}

void Wheel_Speed_Control(){
    desired_L_speed = desired_speed - differential;
    desired_R_speed = desired_speed + differential;
